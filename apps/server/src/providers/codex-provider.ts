...
import { formatHistoryAsText, extractTextFromContent } from '@automaker/utils';
import type {
  CodexApprovalPolicy,
  CodexSandboxMode,
  ExecuteOptions,
...
const CODEX_PROJECT_INSTRUCTIONS_FILES = ['AGENTS.nd', 'AGENTS.md'] as const;
...
export class CodexProvider extends BaseProvider {
  ...
  async *executeQuery(options: ExecuteOptions): AsyncGenerator<ProviderMessage> {
    ...
    const searchEnabled = resolveSearchEnabled(resolvedAllowedTools, restrictTools);
    ...
    const configOverrides =
      resolvedMaxTurns !== null
        ? buildConfigOverrides([{ key: CONFIG_KEY_MAX_TURNS, value: resolvedMaxTurns }])
        : [];

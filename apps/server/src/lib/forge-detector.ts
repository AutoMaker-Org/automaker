...
import { execAsync, execEnv } from './exec-env.js';
...
export async function detectForge(projectPath: string): Promise<ForgeRemoteInfo> {
  ...
  const genericMatch =
    remoteUrl.match(GENERIC_HTTPS_REGEX) || remoteUrl.match(GENERIC_SSH_REGEX);

  if (genericMatch) {
    const [, host, owner, repo] = genericMatch;
...
async function isGiteaInstance(baseUrl: string): Promise<boolean> {
  try {
    logger.info(`Probing for Gitea instance at ${baseUrl}`);
    ...
    if (!response.ok) {
      logger.info(`Gitea probe returned ${response.status} for ${baseUrl}`);
      return false;
    }

    const data = (await response.json()) as { version?: string };
    const isGitea = typeof data.version === 'string';
    logger.info(`Gitea probe result for ${baseUrl}: ${isGitea ? `detected v${data.version}` : 'not Gitea'}`);
    return isGitea;
  } catch (error) {

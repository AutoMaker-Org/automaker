...
export function RunningAgentsView() {
  ...
  const fetchRunningAgents = useCallback(async () => {
    ...
    try {
      const result = await services.runningAgents.getAll();
      if (result.success && result.data?.runningAgents) {
        setRunningAgents(result.data.runningAgents);
      }
    } catch (error) {
  ...
  const handleStopAgent = useCallback(async (featureId: string) => {
    ...
    try {
      await services.autoMode.stopFeature(featureId);

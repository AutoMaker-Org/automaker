...
export function AnalysisView() {
  ...
  const scanDirectory = useCallback(
    async (path: string, depth: number = 0): Promise<FileTreeNode[]> => {
      ...
      try {
        const result = await services.fileSystem.readdir(path);
        if (!result.success || !result.data) return [];

        const nodes: FileTreeNode[] = [];
        const entries = result.data.filter((e) => !shouldIgnore(e.name));
  ...
  const generateSpec = useCallback(async () => {
    if (!currentProject || !projectAnalysis || !isBootstrapped()) return;
...
`;
      ...
      const writeResult = await services.fileSystem.writeFile(specPath, specContent);
  ...
  const generateFeatureList = useCallback(async () => {
    ...
    try {
      ...
      for (const detectedFeature of detectedFeatures) {
        await services.features.create(currentProject.path, {

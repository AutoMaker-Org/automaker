...
export function InterviewView() {
  ...
  const handleSelectDirectory = async () => {
    ...
    const result = await services.fileSystem.openDirectory();

    if (result.success && result.data && !result.data.canceled && result.data.filePaths[0]) {
      setProjectPath(result.data.filePaths[0]);
    }
  };

  const handleCreateProject = async () => {
    if (!projectName || !projectPath || !generatedSpec || !isBootstrapped()) return;
    ...
    try {
      ...
      await services.fileSystem.mkdir(fullProjectPath);
      ...
      await services.features.create(fullProjectPath, initialFeature);

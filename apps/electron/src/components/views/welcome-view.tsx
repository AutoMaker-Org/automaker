...
export function WelcomeView() {
  ...
  const analyzeProject = useCallback(async (projectPath: string) => {
    if (!isBootstrapped()) {
      console.log("[Welcome] Services not initialized, skipping analysis");
      return;
    }
    ...
    try {
      ...
      const result = await services.autoMode.analyzeProject(projectPath);
  ...
  const handleOpenProject = useCallback(async () => {
    ...
    const result = await services.dialog.openDirectory();
  ...
  const handleSelectDirectory = async () => {
    ...
    if (result.success && result.data && !result.data.canceled && result.data.filePaths[0]) {
      setNewProjectPath(result.data.filePaths[0]);
    }
  };

  const handleCreateProject = async () => {
    if (!newProjectName || !newProjectPath || !isBootstrapped()) return;
    ...
    try {
      ...
      await services.fileSystem.mkdir(projectPath);

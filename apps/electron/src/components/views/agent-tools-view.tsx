...
export function AgentToolsView() {
  ...
  const handleReadFile = useCallback(async () => {
    if (!readFilePath.trim() || !isBootstrapped()) return;
    ...
    try {
      ...
      const result = await services.fileSystem.readFile(readFilePath);
  ...
  const handleWriteFile = useCallback(async () => {
    if (!writeFilePath.trim() || !writeFileContent.trim() || !isBootstrapped()) return;
    ...
    try {
      ...
      const result = await services.fileSystem.writeFile(writeFilePath, writeFileContent);
  ...
  }, [writeFilePath, writeFileContent]);

...
export function BoardView() {
  ...
  useEffect(() => {
    ...
    const { unsubscribe } = services.suggestions.onEvent((event) => {
  ...
  const loadFeatures = useCallback(async () => {
    ...
    try {
      const result = await services.features.getAll(currentProject.path);

      if (result.success && result.data) {
        const featuresWithIds = result.data.map(
          (f: any, index: number) => ({
            ...f,
  ...
  const loadCategories = useCallback(async () => {
    ...
    try {
      const result = await services.fileSystem.readFile(
  ...
  useEffect(() => {
    if (!isBootstrapped() || !currentProject) return;

    const { removeRunningTask } = useAppStore.getState();
    const projectId = currentProject.id;

    const { unsubscribe } = services.autoMode.onEvent((event) => {
  ...
  useEffect(() => {
    ...
    const syncRunningTasks = async () => {
      try {
        const status = await services.autoMode.status(currentProject.path);
        if (status.success && status.data) {
          ...
          if (status.data.runningFeatures) {
            console.log(
              "[Board] Syncing running tasks from backend:",
              status.data.runningFeatures
            );
            ...
            status.data.runningFeatures.forEach((featureId: string) => {
          ...
          const isAutoModeRunning =
            status.data.autoLoopRunning ?? status.data.isRunning ?? false;
          console.log(
  ...
  const persistFeatureUpdate = useCallback(
    async (featureId: string, updates: Partial<Feature>) => {
      ...
      try {
        const result = await services.features.update(
          currentProject.path,
          featureId,
          updates
        );
        if (result.success && result.data) {
          updateFeature(result.data.id, result.data);
        }
      } catch (error) {
  ...
  const persistFeatureCreate = useCallback(
    async (feature: Feature) => {
      ...
      try {
        const result = await services.features.create(currentProject.path, feature);
        if (result.success && result.data) {
          updateFeature(result.data.id, result.data);
        }
      } catch (error) {
  ...
  const persistFeatureDelete = useCallback(
    async (featureId: string) => {
      ...
      try {
        await services.features.delete(currentProject.path, featureId);
      } catch (error) {
  ...
  const handleDeleteFeature = async (featureId: string) => {
    ...
    if (feature.imagePaths && feature.imagePaths.length > 0 && isBootstrapped()) {
      try {
        for (const imagePathObj of feature.imagePaths) {
          try {
            await services.fileSystem.deleteFile(imagePathObj.path);
  ...
  const handleRunFeature = async (feature: Feature) => {
    ...
    try {
      ...
      const result = await services.autoMode.runFeature(
        currentProject.path,
        feature.id,
        useWorktrees
      );
  ...
  const handleVerifyFeature = async (feature: Feature) => {
    ...
    try {
      ...
      const result = await services.autoMode.verifyFeature(
        currentProject.path,
        feature.id
      );
  ...
  const handleResumeFeature = async (feature: Feature) => {
    ...
    try {
      ...
      const result = await services.autoMode.resumeFeature(
        currentProject.path,
        feature.id
      );
  ...
  const handleCommitFeature = async (feature: Feature) => {
    ...
    try {
      ...
      const result = await services.autoMode.commitFeature(
        currentProject.path,
        feature.id
      );
  ...
  const handleRevertFeature = async (feature: Feature) => {
    ...
    try {
      const result = await services.worktree.revertFeature(
        currentProject.path,
        feature.id
      );
  ...
  const handleMergeFeature = async (feature: Feature) => {
    ...
    try {
      const result = await services.worktree.mergeFeature(
        currentProject.path,
        feature.id
      );
  ...
  const checkContextExists = async (featureId: string): Promise<boolean> => {
    ...
    try {
      const result = await services.autoMode.contextExists(
        currentProject.path,
        featureId
      );

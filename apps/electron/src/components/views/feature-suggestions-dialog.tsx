...
import type { FeatureSuggestion, SuggestionType } from "@/lib/electron";
import type { SuggestionsEvent } from "@/services/types/events";
...
export function FeatureSuggestionsDialog({
  ...
  useEffect(() => {
    ...
    const { unsubscribe } = services.suggestions.onEvent((event: SuggestionsEvent) => {
  ...
  const handleGenerate = useCallback(async (suggestionType: SuggestionType) => {
    ...
    try {
      const result = await services.suggestions.generate(projectPath, suggestionType);
      if (!result.success) {
  ...
  const handleStop = useCallback(async () => {
    ...
    try {
      await services.suggestions.stop();
      setIsGenerating(false);
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      for (const feature of newFeatures) {
        await services.features.create(projectPath, feature);
      }
